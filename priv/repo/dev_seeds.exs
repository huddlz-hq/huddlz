# Dev-only sample data for local development.
#
# Run with:
#
#     mix run priv/repo/dev_seeds.exs
#
# (Or via the `mix setup` / `mix ecto.setup` aliases, which run this file.)
#
# Production-essential seeds (default roles, lookup tables, etc.) live in
# `priv/repo/seeds.exs` and run in every environment. *This* file is dev
# fixtures: known users, groups, huddlz with fixed emails like
# `alice@example.com`. Those fixed emails would collide with hard-coded
# fixtures in many tests, so this script refuses to run in MIX_ENV=test.

if Mix.env() == :test do
  raise """
  Refusing to run priv/repo/dev_seeds.exs in MIX_ENV=test.

  This file inserts fixed emails (alice@example.com, …) that collide with
  test fixtures. If those rows ever land in `huddlz_test`, every test that
  creates a user with one of those emails fails with "has already been
  taken" because Ash's identity check sees the leaked committed row from
  outside the test's sandboxed transaction.

  Re-run with MIX_ENV=dev (or omit MIX_ENV entirely) to populate the dev
  database.
  """
end
