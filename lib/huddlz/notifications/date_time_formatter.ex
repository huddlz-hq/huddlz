defmodule Huddlz.Notifications.DateTimeFormatter do
  ...
  def format_date_iso(iso, time_zone \\ @default_time_zone, fallback \\ nil)

  def format_date_iso(iso, time_zone, fallback) when is_binary(iso) do
    case DateTime.from_iso8601(iso) do
      {:ok, datetime, _offset} ->
        datetime
        |> shift_or_default(time_zone)
  ...
  def format_date_iso(_, _, fallback), do: fallback
