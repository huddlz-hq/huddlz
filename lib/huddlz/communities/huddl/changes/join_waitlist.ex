defmodule Huddlz.Communities.Huddl.Changes.JoinWaitlist do
  ...
  def change(changeset, _opts, %{actor: %{id: user_id} = actor}) when not is_nil(user_id) do
    Ash.Changeset.before_action(changeset, &queue_spot(&1, actor))
  end
  ...
  defp queue_spot(cs, actor) do
    case LockedHuddl.fetch(cs.data.id, :at_capacity) do
      {:ok, %Huddl{} = huddl} -> queue_spot(cs, huddl, actor)
      error -> LockedHuddl.add_read_error(cs, error)
    end
  end

  defp queue_spot(cs, huddl, %{id: user_id} = actor) do
    cond do
      ...
      true ->
        case fetch_existing(huddl.id, user_id) do
          {:ok, nil} -> create_waitlist_entry(cs, huddl.id, actor)
          {:ok, _existing} -> cs
  ...
  defp create_waitlist_entry(cs, huddl_id, %{id: user_id} = actor) do
    HuddlAttendee
