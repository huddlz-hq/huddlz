...
Feature: Unsubscribe URL
  As a user
  I want a one-click way to opt out of an email category from the email itself
  So that I do not have to sign in just to silence a single trigger

  Scenario: A signed-in user visits a valid unsubscribe URL and the preference flips
