defmodule Huddlz.Notifications.Senders.HuddlUpdatedTest do
  use Huddlz.DataCase, async: true

  alias Huddlz.Mailer
  alias Huddlz.Notifications.Senders.HuddlUpdated
  ...
  describe "build/2" do
    ...
    test "subject says updated and names the huddl" do
      user = generate(user())
      email = HuddlUpdated.build(user, default_payload(%{"huddl_title" => "Saturday Soccer"}))

      assert email.subject == "Updated: Saturday Soccer"
    end
    ...
    test "links to the huddl page" do
      user = generate(user())
      huddl_id = Ash.UUID.generate()
    ...
    test "falls back gracefully when changed_fields is missing" do
      user = generate(user())
      email = HuddlUpdated.build(user, %{})

      assert email.html_body =~ "huddl details"
    end
  end
end
