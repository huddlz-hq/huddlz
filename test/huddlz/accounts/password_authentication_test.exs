defmodule Huddlz.Accounts.PasswordAuthenticationTest do
  ...
  describe "password authentication" do
    test "user resource has hashed_password attribute" do
      assert :hashed_password in (User |> Ash.Resource.Info.attributes() |> Enum.map(& &1.name))
    end

    test "user resource has confirmed_at attribute" do
      assert :confirmed_at in (User |> Ash.Resource.Info.attributes() |> Enum.map(& &1.name))
    end
    ...
    test "change_password action exists" do
      assert :change_password in (User |> Ash.Resource.Info.actions() |> Enum.map(& &1.name))
    end
